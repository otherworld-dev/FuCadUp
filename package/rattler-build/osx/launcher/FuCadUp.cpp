...
int main(int argc, char *argv[], char *const *envp) {
    ...
    std::string executable = realpath((std::string(cwd) + "/../Resources/bin/fucadup").c_str(), NULL);
    ...
    new_argv[0] = new char[executable.length() + 1];
    memset(new_argv[0], 0, executable.length() + 1);
    strncpy(new_argv[0], executable.c_str(), executable.length());
    ...
    return execve(executable.c_str(), new_argv, new_env);
}
