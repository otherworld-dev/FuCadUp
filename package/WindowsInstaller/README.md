# Creating a Windows installer for FuCadUp

These are instructions for building an NSIS-based installer for FuCadUp. They were designed for FuCadUp 0.21 and later,
and presume that you have cloned a copy of FuCadUp's source code, and therefore have the directory *package/WindowsInstaller*.
...
## Build the installer
Next, update the installer settings for the current version of FuCadUp. Starting from the *package/WindowsInstaller* folder in the FuCadUp source tree:
1. Set the appropriate version strings for the release you are creating. These are used to construct the filename of the installer, among other things. If you have to upload a new version of the installer for the exact same release of FuCadUp, increment `APP_VERSION BUILD` as needed. The main version numbers are dynamically obtained by calling `fucadupcmd.exe`.
```nsis
...
```
4. Ensure the FuCadUp files are in place. Here you have two options:
   * If you are working from an already-compiled version of FuCadUp provided to you by an outside source: in this case, simply ensure that `FILES_FUCAD` is set to the directory containing those files.
   * If you compiled FuCadUp on your own as described [here](https://wiki.freecad.org/Compile_on_Windows) (and using the Install option outlined there). Then:
...
5. Right-click on the file *FuCadUp-installer.nsi* and choose **Compile NSIS script**
   to compile the installer. You can also run from command line to specify some settings
```cmd
%your_nsis_path%\makensis.exe /D'FILES_FUCAD="D:\some\path\FuCadUp"' /D'FILES_DEPS="${__FILEDIR__}\MSVC_Redist" /D'ExeFile="my-FuCadUp-installer.exe"' FuCadUp-installer.nsi
```
...
```cmd
%your_nsis_path%\makensis.exe [OPTIONS] /DFC_TEST_BUILD FuCadUp-installer.nsi
```
